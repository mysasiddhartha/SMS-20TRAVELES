    ...
    <header className={cn("sticky top-0 z-40 w-full border-b bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/50 dark:bg-background/70", className)}>
      <div className="container flex h-16 items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="font-extrabold tracking-tight text-xl">Mysa</span>
        </Link>
        <div className="hidden items-center gap-3 sm:flex">
          <a href={`tel:${phone.replace(/[^+\d]/g, "")}`} className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
            {phone}
          </a>
          <a
